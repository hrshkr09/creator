    ...
    return (
        <header className='fixed top-6 left-1/2 transform -translate-x-1/2 z-50 w-full max-w-3xl px-4'>
            <div className='backdrop-blur-md bg-white/10 border border-white/20 rounded-full px-4 sm:px-6 md:px-8 py-3 flex items-center justify-between  gap-2'>
                <Link href={isAuthenticated? "/feed" :"/"} className='flex-shrink-0'><Image src="/logo.png" alt="creator logo" width={96} height={32} className="h-8 sm:h-10 w-auto object-contain" /></Link>

                {
                    path ==="/" && (
                        <div className='hidden lg:flex space-x-6 flex-1 justify-center'>
                            <Link href="#features" className='text-white font-medium transition-all duration-300 hover:text-purple-300 cursor-pointer'>Features
                            </Link>
                             <Link href="#testimonials" className='text-white font-medium transition-all duration-300 hover:text-purple-300 cursor-pointer'>Testimonials
                            </Link>
                        </div>
                    )
                }
